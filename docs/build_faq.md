...
- [FAQ](#faq)
  ...
  - [Windows build](#windows-build-1)
          ...
          - [How do I use PX4 firmware with UNav-Sim?](#how-do-i-use-px4-firmware-with-unav-sim)
  ...
  - [Linux build](#linux-build-1)
          ...
          - [What compiler and stdlib does UNav-Sim use?](#what-compiler-and-stdlib-does-unav-sim-use)
          - [What version of CMake does the UNav-Sim build use?](#what-version-of-cmake-does-the-unav-sim-build-use)
          - [Can I compile UNav-Sim in BashOnWindows?](#can-i-compile-unav-sim-in-bashonwindows)
          - [Where can I find more info on running Unreal on Linux?](#where-can-i-find-more-info-on-running-unreal-on-linux)
  - [Other](#other-1)
          - [Packaging a binary including the UNav-Sim plugin](#packaging-a-binary-including-the-unav-sim-plugin)
...
* [Packaging UNav-Sim](#packaging-a-binary-including-the-airsim-plugin)
...
>
>A common problem could be Visual Studio version conflict, UNav-Sim uses VS 2019 while UE is using VS 2017, this can be found by searching for `2017` in the Log file. In that case, see the answer above.
>
>If you have modified the UNav-Sim plugin files, then you can right-click the `.uproject` file, select `Generate Visual Studio solution file` and then open the `.sln` file in VS to fix the errors and build again.

<!-- ======================================================================= -->

###### I get `error C100 : An internal error has occurred in the compiler` when running build.cmd

>We have noticed this happening with VS version `15.9.0` and have checked-in a workaround in UNav-Sim code. If you have this VS version, please make sure to pull the latest UNav-Sim code.
...
###### How do I use PX4 firmware with UNav-Sim?

>By default, UNav-Sim uses its own built-in firmware called [simple_flight](simple_flight.md). There is no additional setup if you just want to go with it. If you want to switch to using PX4 instead then please see [this guide](px4_setup.md).
...
###### What compiler and stdlib does UNav-Sim use?

>We use the same compiler that Unreal Engine uses, **Clang 8**, and stdlib, **libc++**. UNav-Sim's `setup.sh` will automatically download them.

<!-- ======================================================================= -->

###### What version of CMake does the UNav-Sim build use?
...
###### Can I compile UNav-Sim in BashOnWindows?
...
###### Packaging a binary including the UNav-Sim plugin

>In order to package a custom environment with the UNav-Sim plugin, there are a few project settings that are necessary for ensuring all required assets needed for UNav-Sim are included inside the package. Under `Edit -> Project Settings... -> Project -> Packaging`, please ensure the following settings are configured properly:
>
>- `List of maps to include in a packaged build`: ensure one entry exists for `/UNav-Sim/AirSimAssets` 
>- `Additional Asset Directories to Cook`: ensure one entry exists for `/UNav-Sim/HUDAssets`
