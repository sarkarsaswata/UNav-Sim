# Build UNav-Sim on macOS

Only macOS **Catalina (10.15)** has currently been tested. Theoretically, UNav-Sim should work on higher macOS versions and Apple Silicon hardware, but this path is not offically supported.
...
2. Run the Epic Games Launcher, open the `Library` tab on the left pane.
   ...
   **Note**: UNav-Sim also works with UE >= 4.24, however, we recommend 4.27.
...
By default UNav-Sim uses clang 8 to build for compatibility with UE 4.25. The setup script will install the right version of cmake, llvm, and eigen.
...
- Browse to `UNav-Sim/Unreal/Environments/Blocks`.
